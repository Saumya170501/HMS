...
export default function Sidebar() {
    ...
    return (
        <aside className="sidebar-rail sidebar-transition bg-surface border-r border-border flex-col h-screen hidden md:flex md:w-64 md:sticky md:top-0 z-50">
            {/* Logo */}
            <div className="p-4 border-b border-border">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center shadow-lg shadow-blue-500/20 flex-shrink-0">
                ...
                <ul className="space-y-1">
                    {navItems.map((item) => (
                        <li key={item.path}>
                            ...
                            >
                                <item.icon className="w-5 h-5 transition-transform group-hover:scale-105 flex-shrink-0" />
                                <span className="nav-label font-medium text-sm">{item.label}</span>
                            </NavLink>
                        </li>
                    ))}
                </ul>
